...
router.get('/', function(req, res, next) {
    ...
    if (!req.isAuthenticated()) { res.redirect('/'); }
     pg.connect(connectionString, function(err, client, done) {
        ...
        var results = [];
        var query = client.query("SELECT group_name, group_description, group_start_date, group_end_date, subject_name FROM db_group INNER JOIN db_group_user ON db_group.group_id = db_group_user.group_id INNER JOIN db_subject on db_group.subject_id = db_subject.subject_id");
